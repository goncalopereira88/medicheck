...
- **base64 por blocos na encriptação** (2026-08-19): `bytesToBase64()` converte em blocos de 32 KB. NUNCA usar `String.fromCharCode(...bytes)` — o spread passa um argumento por byte e lança `RangeError: Maximum call stack size exceeded` assim que os dados encriptados passam ~100 KB (≈300-400 actos). Foi esta a causa real do sync com o Drive ter parado; o `catch` mudo do `saveToDrive` escondeu-a durante semanas. `decryptData` já usava `Uint8Array.from(atob(...))`, sem spread — por isso a leitura funcionava e só a escrita falhava
- **Ciclo de vida do token Google reforçado** (2026-08-19): `ensureToken()` valida a validade do token antes de *cada* chamada ao Drive (o `setTimeout` de renovação é estrangulado em background no mobile e muitas vezes não dispara); `driveFetch()` injecta o header e repete o pedido uma vez em 401/403; `silentRefreshToken()` devolve Promise com timeout de 15s e guard `settled` — a flag `_silentRefresh` já não fica presa a `true` (bug que bloqueava todos os logins seguintes); `requestGoogleLogin()` limpa o estado de refresh; PATCH com 404 recria o ficheiro no Drive; banner de sync mostra o código HTTP real; botão do banner usa `reauthSync()` (refresh silencioso com gesto do utilizador, consentimento só como fallback)
